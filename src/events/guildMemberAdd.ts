...
export async function execute(member: any): Promise<void> {
  const cfg = loadGuilds().get(member.guild.id) ?? {} as any;
  // Welcome
  ...
  if (cfg.autoRoles?.length) {
    for (const roleId of cfg.autoRoles) {
      try { const role = member.guild.roles.cache.get(roleId); if (role && role.editable) await (member as any).roles.add(roleId); } catch {}
    }
  }
